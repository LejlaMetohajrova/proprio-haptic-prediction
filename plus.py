...
class Perceptron:
    ...
    def read_data(self):        
        x1 = np.arange(200).reshape(200, 1)/400
        x2 = np.linspace(-0.5, 0.5, num=200).reshape(200, 1)
        ...
        self.Y = np.sum(self.X, axis=1).reshape(200, 1)
    ...
    def eval_hidden_layer(self):
        self.hidden_layer = self.act(np.dot(self.input_layer, self.weights0), type='tanh')
        
    def eval_output_layer(self):        
        self.output_layer = self.act(np.dot(self.hidden_layer, self.weights1), type='tanh')
            
    def train(self, X, Y, alpha=0.001, number_of_epochs=1001):      
        ...
        for i in range(number_of_epochs):
            ...
            hidden_delta = output_delta.dot(self.weights1.T) * self.deriv(self.hidden_layer, type='tanh')
