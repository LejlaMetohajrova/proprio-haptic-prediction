...
def binarize(contact_vector):
    ...
    for v in contact_vector:
        if v[3] == 1:
            for t in v[19:-1]:
                bin[[i for i in range(0, taxel.shape[1]) if t in taxel[0][i]]] = 1
        elif v[3] == 2:
            for t in v[19:-1]:
                bin[[i + 10 for i in range(0, taxel.shape[1]) if t in taxel[1][i]]] = 1
        elif v[3] == 4:
            for t in v[19:-1]:
                bin[[i + 20 for i in range(0, taxel.shape[1]) if t in taxel[0][i]]] = 1
        elif v[3] == 5:
            for t in v[19:-1]:
                bin[[i + 30 for i in range(0, taxel.shape[1]) if t in taxel[1][i]]] = 1
    return bin
